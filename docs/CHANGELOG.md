...
- **Project files, photos and backups in Google Drive (v1.12.0)**
  - **Files and photos on every project.** Editors, managers and the owner can add
    ...
    delete their own files, managers any file. Up to 7 MB per file. Site photos in
    JPEG, PNG or WebP over 2 MB are made smaller (at most 2400 px) on the device
    first, which also strips the location data inside them; drawings, certificates
    and other files keep their full detail.
  - **Kept in the portal's Google Drive.** Each project gets its own folder
    ...
    `DRIVE_PROJECT_QUOTA_MB` (2 GB per project by default) and
    `DRIVE_USER_QUOTA_MB` (10 GB per person) in `functions/.env`.
  - **Limits that cannot be dodged.** Each project holds at most 2 GB and 5,000
    files; each person can add at most 10 GB and 20,000 files across all their
    projects. Deleted files stay in the Drive trash for 30 days and keep counting
    until the nightly job frees them. Space is reserved before a file is sent to
    Drive, so two uploads at once cannot both slip past a limit.
  ...
  - **Backups.** Every night at 02:00 UAE time, each project that changed is backed
    ...
    before-restore backups. **Back up now** and **Restore items** work once a
    minute per project. Projects over 3,000 items or 5,000 files are not backed up;
    backups over 7 MB are downloaded from Drive rather than the app. The nightly
    job skips unchanged projects cheaply, stops starting new ones after about 7
    minutes, and continues oldest-first the next night.
  - **Privacy and Terms** describe project files and backups, so everyone is asked
    to accept them again once.
  - **Tests.** 20 unit tests for the file and backup rules (`test/files.test.mjs`),
    5 new Firestore rules tests, 15 tests of the Drive and Firestore work against
    the Firestore emulator (`test/project-drive.test.mjs`: space and file limits,
    two uploads at once, the 30-day trash hold, Drive failures half-way, the
    once-a-minute limits, nightly skipping and time budget), and 4 new browser
    test steps (upload, photo shrinking, refused files, download and view, item
    attachments, backup, restore) run against a local stand-in for Drive.
