...
(function (root) {
  ...
  const TYPES = [
    ...
    },
    {
      ...
      maxValidityHours: 12,
      isolations: true,
      isolationHint: 'List every energy source, including stored energy. Each point needs its own lock and tag, and zero energy must be proven before work starts.',
      energyTypes: ['Electrical', 'Mechanical', 'Hydraulic', 'Pneumatic', 'Chemical / process', 'Thermal', 'Stored energy (springs, capacitors)', 'Gravity'],
      fields: [
        { key: 'equipment', label: 'Equipment or system isolated, with tag number', kind: 'text', required: true },
        ...
        { key: 'zeroEnergyMethod', label: 'How zero energy is proven', kind: 'text', required: true,
          hint: 'For example voltage test at the motor terminals, try-start from the local control, pressure gauge at zero' },
        ...
        { key: 'workers', label: 'Workers covered by these isolations', kind: 'list', required: true, group: 'People' },
      ],
      checks: [
        BRIEFED,
        NEARBY,
        { key: 'sourcesIdentified', label: 'All energy sources identified from drawings and on site, including stored energy', required: true },
        ...
        { key: 'lockedTagged', label: 'Every isolation point locked and tagged with name, date and reason', required: true },
        { key: 'storedReleased', label: 'Stored energy released: pressure bled, capacitors discharged, moving parts blocked', required: true },
        { key: 'zeroEnergy', label: 'Zero energy proven by test or try-start, then controls put back to off', required: true },
      ...
      rules: [
        requireCheckWhen('testerChecked', (p) => (p.isolations || []).some((r) => r && r.energy === 'Electrical')),
        requireCheckWhen('processIsolation', (p) => (p.isolations || []).some((r) => r && /Chemical|Hydraulic|Pneumatic|Thermal/.test(r.energy || ''))),
      ],
      closeout: [
        { key: 'workDone', label: 'Work finished; tools removed; guards and covers refitted', required: true },
        { key: 'peopleClear', label: 'Everyone clear of the equipment and told it will be re-energised', required: true },
        { key: 'locksRemoved', label: 'Each lock and tag removed by the person who fitted it', required: true },
        { key: 'returned', label: 'Equipment restored in sequence, tested and handed back to operations', required: true },
        { key: 'registerUpdated', label: 'Isolation register or lock log updated' },
      ],
    },
    {
      key: 'general', label: 'General work', prefix: 'GW',
      summary: 'Maintenance, construction and other work that needs control but no special permit.',
      ...
      fields: [
        ...
        { key: 'tools', label: 'Tools, plant and equipment used', kind: 'text' },
        { key: 'areaOwner', label: 'Area owner or authority who agreed the work', kind: 'text', required: true },
        { key: 'workers', label: 'Work party', kind: 'list', required: true, group: 'People' },
      ],
      checks: [
        BRIEFED,
        NEARBY,
        { key: 'areaInspected', label: 'Work area inspected with the area owner', required: true },
        { key: 'ppe', label: 'Required PPE available and worn', required: true },
        ...
        { key: 'emergencyKnown', label: 'Emergency arrangements, alarms and assembly point known', required: true },
        { key: 'housekeeping', label: 'Access routes kept clear; waste and materials stored safely' },
        { key: 'heatStress', label: 'Heat stress controls in place: water, rest, shade, midday break' },
      ],
      closeout: [
        { key: 'workDone', label: 'Work finished and inspected', required: true },
